...
pipeline below (TIFF header, IFD tags, tile lookup, decompression,
predictor, georeferencing) was verified end-to-end in a local sandbox
test against a hand-constructed synthetic COG-like file built with the
same encoding rules -- all pixel lookups (tile corners, interior points,
both tiles, out-of-bounds) matched exactly. Two real bugs were found and
fixed by this testing: (1) reading a tag's externally-stored value moved
the file pointer, so the NEXT tag was read from the wrong position --
fixed by seeking to each IFD entry's absolute computed position instead
of relying on sequential reads; (2) float division landing a hair below
an intended integer pixel index (e.g. 2.999999999 instead of 3.0) was
truncated by int() to the WRONG neighboring pixel -- fixed by using
round() instead. NOT yet tested against a real Copernicus-produced file
(only a self-built synthetic one) -- that is the next honest step before
this is trusted on-device.
"""
...
class CopernicusDemTile:
    ...
    file -- offline_dem_store.py (not yet written) is responsible for
    picking the right tile file for a given coordinate and constructing
    this."""
    ...
    def get_elevation(self, lon: float, lat: float) -> Optional[float]:
        ...
        with open(self._file_path, 'rb') as f:
            tile = _read_tile(f, self._layout, tile_index)
