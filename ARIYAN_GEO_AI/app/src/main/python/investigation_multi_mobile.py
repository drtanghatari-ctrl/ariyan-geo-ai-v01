...
def run_investigation_multi_json(
    ...
    separate toggle for either, same as NDVI itself has none).
    ...
    fifth_evidence: object = RealOpticalCoreHaloEvidence(
    ...
    if dem_candidates and n_ndvi_errors == n_candidates:
        ...
        # to the offline raster/geometric path. Thermal's and Optical's
        # own results are UNCHANGED by this (still fully recorded below
        # in fourth_evidence_detail / fifth_evidence_detail) but are
        # deliberately not woven into this fallback's own
        # supporting_sources/notes -- see this module's docstring for why.
        try:
        ...
        # candidate. Combine NDVI + Thermal + Optical per-candidate, fully.
        second_anomalies = ndvi_results
        second_anomalies_are_candidates = False
        correlation_results = _build_correlated_candidates(
            dem_candidates, ndvi_results, thermal_results, optical_results,
