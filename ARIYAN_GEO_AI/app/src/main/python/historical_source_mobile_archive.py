...
RECOVERED 2026-09-17: this real, working module's content had been
silently committed under historical_source_mobile_wikipedia.py's
filename instead of its own (confirmed via a real Python import test --
the file previously named "wikipedia" defined
fetch_historical_evidence_archive_safe, not the wikipedia function
combined.py actually calls). Delivered here under its correct real
filename, content otherwise unchanged from what was already live-tested
during original development. See historical_source_mobile_wikipedia.py's
own docstring for the matching note and the real replacement module.

Second of three independent, self-contained keyless historical-evidence
