 ...
 *
 * When "Include NDVI correlation" is on, NDVI corroboration has two modes:
 *
 *  - Synthetic (default): an independent synthetic NDVI raster is
 *    correlated against DEM anomalies. Always labeled synthetic on screen.
 *  - Real (opt-in via "Use real NDVI"): a real, TARGETED per-DEM-candidate
 *    Sentinel-2 core/halo check via the Copernicus Data Space Ecosystem
 *    Statistical API (investigation_multi_mobile.py's real_ndvi branch).
 *    Requires the user's own Copernicus OAuth2 client ID + secret, entered
 *    in-app and held only in memory for this session -- never written to
 *    disk, same pattern as the OpenTopography API key.
 *
 * After a successful investigation, the AI Debate Engine
 * (debate_mobile.run_debate_json()) runs automatically on the returned
 * evidence record and its per-candidate positions/synthesis are appended
 * to the results view. The debate engine is rule-based and fully offline
 * (no LLM, no network, no API key) -- see debate_engine.py. A debate
 * failure never blocks showing the investigation results themselves.
 *
 * There is no depth estimation in this build; see HANDOFF.md for the
 * honest current state and what's next.
 */
class MainActivity : AppCompatActivity() {
    ...
    private fun onRunClicked() {
        ...
        if (includeNdvi && useRealNdvi && (ndviClientId.isEmpty() || ndviClientSecret.isEmpty())) {
            toast("Enter your Copernicus client ID and secret, or turn off \"Use real NDVI\""); return
        }
        ...
        lifecycleScope.launch {
            try {
                val json = withContext(Dispatchers.Default) {
                    runInvestigation(
                        lat, lon, radius, grid, useRealDem, apiKey, demType,
                ...
                // is safe to run automatically after every investigation.
    ...
    private fun runInvestigation(
        ...
        useRealNdvi: Boolean, ndviClientId: String, ndviClientSecret: String
    ): String {
