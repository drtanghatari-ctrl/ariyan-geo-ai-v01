 ...
 *
 * NDVI is still SYNTHETIC on this build (see the "synthetic=" label in
 * the results view) -- real Sentinel-2 NDVI via the Copernicus Statistical
 * API (ndvi_source_mobile.py) is built and tested but NOT yet wired into
 * this Activity; that needs a UI review pass (activity_main.xml) plus a
 * change to investigation_multi_mobile.py, neither of which were
 * available when that module was built. There is no depth estimation in
...
class MainActivity : AppCompatActivity() {
    ...
    private fun onRunClicked() {
        ...
        if (useRealNdvi && (ndviClientId.isEmpty() || ndviClientSecret.isEmpty())) {
        ...
        lifecycleScope.launch {
            try {
                val json = withContext(Dispatchers.Default) {
                    runInvestigation(
                        lat, lon, radius, grid, useRealDem, apiKey, demType, includeNdvi,
                        useRealNdvi, ndviClientId, ndviClientSecret
                    )
                }
